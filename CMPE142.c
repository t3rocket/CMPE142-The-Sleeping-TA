...
void* teacher(void *arg)
{
	...
	while(1){
		...
		printf("\n\tTA is helping student %d from seat #%d",seats[next_stud],(next_stud+1));
		occupied--;
...
void* student(void *thread_id)
{
	...
	while(1)
	{
		...
		sleep(coding_time);
		printf("\nStudent %d is going to TA for help #%d .",id,(help_count+1));
		...
		else
		{
			...
			occupied++;
			printf("\nStudent %d is waiting for TA in seat #%d",id,(empty_chair+1));
